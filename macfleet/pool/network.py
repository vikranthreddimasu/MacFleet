...
class NetworkTopology:
    ...
    def best_link(self) -> Optional[NetworkLink]:
        ...
        if not scored:
            if not self.links:
                return None
            # Before active measurement (score==0 for all links), prefer links
            # that are typically best for distributed training.
            fallback_priority = {
            ...
            return max(
                self.links,
                key=lambda l: (
                    fallback_priority.get(l.link_type, 0),
                    l.theoretical_bandwidth_mbps,
                ),
            )
